...
Values that exist only for construction are globals too: sketch overruns that make a profile reach past the part, such as `pocket_runout`. Label them in the header as non-design values, so nobody edits one expecting the part to change.
